...
function reducer(state, action) {
  // only if active account we should be able to set up other types than openAccount
  ...
  switch(action.type) {
    ...
    case 'withdraw':
      return {...state, balance: state.balance - action.payload};
    case 'requestLoan':
      // if a loan was already requested, cannot access another one
...
function App() {
  // we destructure initial state inti: balance, loan and isActive
  const [{balance, loan, isActive}, dispatch] = useReducer(reducer, initialState);

  return (
    <div className="App">
      <h1>React useReducer Bank Account</h1>
      <p>Balance: {balance}</p>
      <p>Loan: {loan}</p>
